...
return {
	...
	run_test_async = function()
		...
		package.loaded["snacks"] = {
			picker = {
				pick = function(config)
					...
					assert(#config.items == 2, "Snacks: Wrong item count")
		...
		vim.ui.select = function(list, _config, on_choice)
			...
			-- Simulate selection (Native only supports default selection)
			-- Note: code wraps the callback in vim.schedule,
			-- so we must wait or mock schedule.
			on_choice(list[1])
		end
